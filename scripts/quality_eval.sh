...
N_PROTEINS=${N_PROTEINS:-20}   # 단백질 수
N_SEQS=${N_SEQS:-32}           # 단백질당 생성 시퀀스 수
N_STEPS=${N_STEPS:-100}        # ODE 스텝 수
TEMPERATURE=${TEMPERATURE:-0.0}  # 0 = deterministic ODE

# reconstruct : real msa_emb → SFM Decoder  (SFM Decoder 단독 테스트)
# zeroshot    : ESM emb → Latent FM → SFM Decoder  (전체 파이프라인)
